...
final class ProjectDetailsManager {
    ...
    private let gitCommandExecutor: @Sendable (String, [String]) async -> String?
    ...
    init(
        gitCommandExecutor: @escaping @Sendable (String, [String]) async -> String? = { projectPath, arguments in
            await ProjectDetailsManager.runGitCommand(projectPath: projectPath, arguments: arguments)
        },
    ) {
    ...
    private nonisolated static func loadGitSensemakingContext(
        projectPath: String,
        limit: Int,
        executor: @escaping @Sendable (String, [String]) async -> String?,
    ) async -> GitSensemakingContext {
        await _Concurrency.Task.detached(priority: .utility) {
            async let recentFiles = recentFiles(projectPath: projectPath, limit: limit, executor: executor)
            async let gitBranch = gitBranch(projectPath: projectPath, executor: executor)
            async let lastCommitMessage = lastCommitMessage(projectPath: projectPath, executor: executor)
            return await GitSensemakingContext(
    ...
    private nonisolated static func recentFiles(
        ...
        guard let output = await executor(projectPath, ["diff", "--name-only", "HEAD~3", "HEAD"]) else {
            return []
        }
            ...
            process.terminationHandler = { process in
                ...
                if process.terminationStatus == 0, !output.isEmpty {
                    ...
                    continuation.resume(throwing: NSError(
                        ...
                        userInfo: [NSLocalizedDescriptionKey: errorMessage?.isEmpty == false ? errorMessage! : "Haiku generation failed"],
                    ))
                }
            }
