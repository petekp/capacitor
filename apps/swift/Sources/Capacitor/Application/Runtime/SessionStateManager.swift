...
extension SessionStateManager: RuntimeSessionStateProjecting {
    func sessionState(for projectPath: String) -> ProjectSessionState? {
        getSessionState(for: ProjectPathReference(path: projectPath))
    }

    func preferredSessionID(for projectPath: String) -> String? {
        getPreferredSessionId(for: ProjectPathReference(path: projectPath))
    }
    ...
    func applyRuntimeProjectStates(
        ...
        applyRuntimeProjectStates(
            runtimeProjects,
            for: projectPaths.map { ProjectPathReference(path: $0) },
