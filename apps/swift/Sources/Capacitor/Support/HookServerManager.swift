...
struct HookServerManagerDependencies {
    ...
    var findManagedServerProcessForPort: (UInt16, String) async -> Int32?
...
final class HookServerManager {
    ...
    private var startupProbeTask: _Concurrency.Task<Void, Never>?
    ...
    func startIfNeeded() {
        ...
        probeForExistingLiveServerOrLaunch()
    }
    ...
    private func probeForExistingLiveServerOrLaunch() {
        lifecycleGeneration &+= 1
        cancelHealthCheck()
        startupProbeTask?.cancel()
        startupProbeTask = nil
        ...
        let generation = lifecycleGeneration
        startupProbeTask = _Concurrency.Task { [weak self] in
            guard let self else { return }
            let existingPid = await dependencies.findManagedServerProcessForPort(port, binaryPath)
            await MainActor.run {
                self.finishLiveServerProbe(existingPid: existingPid, generation: generation)
            }
        }
    }

    private func finishLiveServerProbe(existingPid: Int32?, generation: UInt64) {
        ...
        if let existingPid, dependencies.isProcessAlive(existingPid) {
            DebugLog.write("HookServerManager: adopting existing live server pid \(existingPid) on port \(port) without pid file")
            beginLifecycleObservation(adoptedPid: existingPid)
            return
        }
...
extension HookServerManagerDependencies {
    static let live = HookServerManagerDependencies(
        ...
        findManagedServerProcessForPort: { port, expectedBinaryPath in
            let process = Process()
                ...
                process.terminationHandler = { proc in
                    ...
                    for line in output.split(separator: "\n") {
                        guard let pid = Int32(line.trimmingCharacters(in: .whitespacesAndNewlines)),
                              let actualPath = LiveHookServerProcess.executablePath(pid: pid),
                              HookServerManager.pathsReferToSameExecutable(actualPath, expectedBinaryPath)
                        else {
                            continue
                        }
        ...
        isManagedServerProcess: { pid, expectedBinaryPath in
            guard let actualPath = LiveHookServerProcess.executablePath(pid: pid) else {
                return false
            }
            return HookServerManager.pathsReferToSameExecutable(actualPath, expectedBinaryPath)
        },
        terminatePid: { pid in
            _ = kill(pid, SIGTERM)
        },
...
extension HookServerManager {
    nonisolated static func pathsReferToSameExecutable(_ lhs: String, _ rhs: String) -> Bool {
        canonicalExecutablePath(lhs) == canonicalExecutablePath(rhs)
    }

    private nonisolated static func canonicalExecutablePath(_ path: String) -> String {
