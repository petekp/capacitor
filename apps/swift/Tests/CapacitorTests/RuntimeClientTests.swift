...
final class RuntimeClientTests: XCTestCase {
    ...
    func testFetchCoreRoutingDiagnosticsIncludesTmuxClientEvidenceForPaneRoute() async throws {
        let client = try makeClient()
        ...
        assertAttachedTmuxRoute(diagnostics.snapshot)
        XCTAssertEqual(diagnostics.snapshot.evidence.count, 1)
        XCTAssertEqual(diagnostics.snapshot.evidence.first?.evidenceType, "tmux_client")
        XCTAssertEqual(diagnostics.snapshot.evidence.first?.value, "/dev/ttys099")
    }

    func testFetchCoreRoutingDiagnosticsFallsBackToRouteHostTtyWhenShellClientTtyMissing() async throws {
        let client = try makeClient(
            coreSnapshot: Self.makeCoreSnapshotResponse(shellTmuxClientTty: nil),
        )
