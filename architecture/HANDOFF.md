...
- Completed `TE-406` by making the explicit end-state decision that `architecture/` remains the permanent architecture-governance surface.
...
- Completed `SM-600` by opening the structure-modernization tranche and measuring the remaining migration-shaped names.
- Completed `SM-601` by renaming the live Rust bounded-context namespace from `clean/` to `contexts/` and renaming the shell/composition type to `ContextServices`.
- Completed `SM-602` by renaming the repo-level governance surface from `rewrite/` to `architecture/` and retargeting CI/docs/scripts to the permanent name.

### Now True
- The architecture control plane no longer treats the March 8 "finish line" as the final architectural standard for Swift.
...
- `scripts/architecture/check_architecture_guards.sh --status` passes with the new convergence budgets, and `cd apps/swift && swift test` passes after the hygiene deletions.
...
- `scripts/architecture/check_architecture_guards.sh` now freezes the last nested `Models/WindowAnchoring/*` subtree and the remaining historical checkpoint TODO rows.
- `scripts/architecture/check_architecture_guards.sh` now passes with `swift_models_window_anchoring_files: 0/0` and `historical_checkpoint_user_run_todos: 0/0`.
...
- `Composition/AppState.swift` is explicitly retained as the shell environment hub, and `architecture/` is explicitly retained as permanent governance.
...
- the Rust live bounded-context code now lives under `core/capacitor-core/src/contexts/`
- the permanent governance surface now lives under `architecture/` with `scripts/architecture/check_architecture_guards.sh`
The finish-line tranche is closed, the first pristine-sweep dead-code tranche is also closed, and the structure-modernization tranche is closed. The remaining nonzero budgets in runtime-reliability are retained migration-regression guards, not open finish-line debt.

### Next Steps
1. Run `scripts/architecture/check_architecture_guards.sh --status`, `scripts/ci/runtime-reliability-guard.sh --status`, and `cargo test -p capacitor-core replay_diff` to confirm the closed modernization baseline.
2. Treat `docs/audit/TRUE_ENDING_AUDIT_2026-03-08.md`, `docs/audit/DEAD_CODE_SWEEP_AUDIT_2026-03-08.md`, and `docs/audit/STRUCTURE_MODERNIZATION_AUDIT_2026-03-08.md` as the canonical closure documents for the migration, pristine sweep, and structure modernization.
3. Future repo changes should start new tranches only for genuinely new debt, not reopen the closed migration-era slices.
