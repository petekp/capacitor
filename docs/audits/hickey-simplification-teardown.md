...
The teardown was executed on branch `decomplect/hickey-teardown` in phased commits (Phase 0 → 5). Every confirmed/actionable finding was landed except the three below, which are **intentionally deferred with rationale** (each is medium-confidence and scope/sequencing-sensitive, not forgotten). The rejected "simplifications" in §7 were correctly left unactioned.

- **C5 — delete `WorkBatchClaudeProcessScanner` (the `ps`/`lsof` scrape) + the synthetic "(duplicate process)" marker.** *Shipped (with one consumer consciously retired).* The audit's prerequisite was built first: hud-hook now runs a periodic OS sweep and the runtime emits a per-session OS-liveness fact `os_process_alive: Option<bool>` (distinct from event-decay `is_alive`), so the scanner was deleted. Of the scanner's **two** consumers, only **one** survives: the quiet-but-live cockpit stays `.ready` after event-decay because Swift now reads `osProcessAlive` instead of scraping `ps`/`lsof`. The **second consumer — same-session duplicate-process detection — was RETIRED, not ported.** A short-lived `os_process_count: Option<u32>` field was added alongside `os_process_alive`, then deleted as vestigial: the runtime keys `SessionSummary` by session-id with exactly one PID each, so a fact built from tracked PIDs can structurally never count `>1` per session-id — `os_process_count` was an exact alias for `(os_process_alive ? 1 : 0)` and could never do what its name implied. The product decision is final: same-session duplicate detection is gone. **Foreign-session** duplicate detection (two distinct runtime session-ids inside one Batch Worktree) still flags and blocks re-entry via `sessionIDs`, and the quiet-cockpit-stays-`.ready` behavior survives via `os_process_alive` — both are pinned by tests.
- **C2-Phase2 — converge the path canonicalizers + add `workspace_id` to the `Project` FFI record.** *Deferred.* C2-Phase1 (the Rust↔Swift conformance test) **was landed** (Phase 4) and made the drift loud: it pins three real cross-language `workspace_id` divergences (`trailing_slash`, `double_trailing_slash`, `git_project_id`) as `documented_divergence` with `phase2_reconcile: true` markers, plus a named test for the Rust-internal `default_workspace_id` vs git-aware `workspace_id` divergence. Convergence is not a naive merge — the three normalizers do genuinely different jobs (Rust reduce-side **pure string** for replay determinism; hud-hook **FS-touching** capture-time; Swift FS+env-dependent), so the correct target is a responsibility *split* (one shared pure-string matcher exported via UniFFI + a separate capture-time canonicalizer), and the join-key change risks detaching sessions. It must first reconcile the pinned Rust-internal key divergence and run green across the corpus. Tracked by the `phase2_reconcile` markers in `core/capacitor-core/tests/fixtures/path_identity_corpus.json`.
- **D4-Tier2 — make `MutateRunCommand` a Rust enum-with-fields-per-kind sum type.** *Deferred.* D4-Tier1 (per-kind Swift factories on `RuntimeRunMutationRequest`) **was landed** (Phase 5), capturing most of the call-site safety win. Tier 2 is the audit's "largest blast radius" item (~1500 lines across `domain/types.rs` + the reducer match + 12+ Rust test files + a full UniFFI regen + all Swift callers), graded optional/medium-confidence and lowest impact-over-effort (roadmap item #20). The flat struct's overspecification is a readability smell, not a correctness bug; best done as a dedicated focused effort.
