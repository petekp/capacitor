...
## 9. Deferred Follow-ups (teardown execution, 2026-05-28)

The teardown was executed on branch `decomplect/hickey-teardown` in phased commits (Phase 0 → 5). Three medium-confidence, scope/sequencing-sensitive findings were **initially deferred** with rationale (not forgotten); all three were **subsequently shipped** on branch `decomplect/deferred-followups` and are recorded below with their landing commits. The rejected "simplifications" in §7 were correctly left unactioned.

- **C5 — delete `WorkBatchClaudeProcessScanner` (the `ps`/`lsof` scrape) + the synthetic "(duplicate process)" marker.** *Shipped (`3df5f372`, with one consumer consciously retired).* The audit's prerequisite was built first: hud-hook now runs a periodic OS sweep and the runtime emits a per-session OS-liveness fact `os_process_alive: Option<bool>` (distinct from event-decay `is_alive`), so the scanner was deleted. Of the scanner's **two** consumers, only **one** survives: the quiet-but-live cockpit stays `.ready` after event-decay because Swift now reads `osProcessAlive` instead of scraping `ps`/`lsof`. The **second consumer — same-session duplicate-process detection — was RETIRED, not ported.** A short-lived `os_process_count: Option<u32>` field was added alongside `os_process_alive`, then deleted as vestigial: the runtime keys `SessionSummary` by session-id with exactly one PID each, so a fact built from tracked PIDs can structurally never count `>1` per session-id — `os_process_count` was an exact alias for `(os_process_alive ? 1 : 0)` and could never do what its name implied. The product decision is final: same-session duplicate detection is gone. **Foreign-session** duplicate detection (two distinct runtime session-ids inside one Batch Worktree) still flags and blocks re-entry via `sessionIDs`, and the quiet-cockpit-stays-`.ready` behavior survives via `os_process_alive` — both are pinned by tests.
- **C2-Phase2 — converge the path canonicalizers + add `workspace_id` to the `Project` FFI record.** *Shipped (`8ad88087`).* C2-Phase1 (the Rust↔Swift conformance test, Phase 4) had made the drift loud by pinning three cross-language `workspace_id` divergences (`trailing_slash`, `double_trailing_slash`, `git_project_id`) for reconciliation. Phase 2 converged them via a responsibility *split*, not a naive merge: `default_workspace_id` is now git-aware (it resolves project identity, then hashes via a normalize-before-hash `workspace_id`), and the pure-string matcher is exported **once** over UniFFI (`normalize_path_for_matching`) so Swift joins on the **Rust-derived** key (`Project.workspace_id`, now on the FFI record) instead of re-deriving it. Swift keeps its own symlink resolution and the `matchesProject` path-containment fallbacks (essential complexity, deliberately not removed). The corpus `phase2_reconcile` rows flipped from `documented_divergence` to `must_agree` and run green, and the Rust-internal `default_workspace_id`-vs-git-aware key divergence is gone. Tracked corpus: `core/capacitor-core/tests/fixtures/path_identity_corpus.json`.
- **D4-Tier2 — make `MutateRunCommand` a Rust enum-with-fields-per-kind sum type.** *Shipped (`d92ae737`; test-target fix `c457c141`).* D4-Tier1 (per-kind Swift factories on `RuntimeRunMutationRequest`, Phase 5) had captured the call-site win; Tier 2 finished the type. `RunMutationKind` is now an enum-with-fields (one variant per kind) and `MutateRunCommand` is a thin `{ project_path, run_id, kind }` record. The audit's flagged overspecification — plus a serde fragility discovered during execution (internally-tagged + `flatten` content-buffering) — were resolved by a hand-written `Serialize`/`Deserialize` projecting through a flat wire DTO, so the on-the-wire shape stays flat while the in-memory type is a proper sum type. The reducer destructures every variant, and a wire-contract oracle (`core/capacitor-core/tests/run_mutation_wire_contract.rs`) pins deserialization, serialization, and per-kind reducer behavior across all 16 kinds plus the `checkpoint_decision_relay` asymmetry. Shipped with a full UniFFI regen and all Swift callers migrated (incl. `WebCaptureServiceTests`).
