...
- **C5 — delete `WorkBatchClaudeProcessScanner` (the `ps`/`lsof` scrape) + the synthetic "(duplicate process)" marker.** *Deferred.* Gated by the audit (roadmap step 18: "only after hud-hook emits OS liveness"). Recon confirmed the prerequisite does not exist: hud-hook drops `proc_start` as `_proc_start` and never emits OS-process liveness on the wire, and emitting it for a *quiet* cockpit needs a new periodic OS sweep in the long-lived service (a genuine new Rust feature, not decomplecting). The scanner also has **two** load-bearing consumers — duplicate-process detection *and* keeping a quiet-but-live cockpit `.ready` after event-decay — so deleting it before the liveness fact exists is a regression, not a simplification. `WorkBatchClaudeProcessScanner.swift` is deliberately left present.
