...
| **Pending Marker** | A JSON file written to `~/.capacitor/runtime/checkpoint-bridge/<run_id>/<checkpoint_id>.pending.json` when a gate fires. Signals to the Swift UI that a decision is needed. | `CheckpointBridgePending` (`checkpoint_bridge_protocol.rs:38`) | Lock file, signal file |
...
| **Relay** | The runtime service mutation path. `BridgeInteractiveIO.post_checkpoint()` sends an `EmitCheckpoint` `MutateRunCommand` to the runtime service, which updates `RunState.active_checkpoint`. The Swift UI reads this via polling. | `RuntimeServiceEndpoint.mutate_run()` | RPC, API call |
...
- The Swift UI reads the **ActiveCheckpoint** and, when decided, writes a **Decision File** that the bridge polls.
