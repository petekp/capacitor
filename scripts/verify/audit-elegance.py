...
def _audit_code_module(
    ...
    if nesting_depth(content) > nesting_threshold:
        score_delta -= weight_for(config, "nesting_depth")
        deductions.append(
            Violation(
                layer="3",
                rule="nesting_depth",
    ...
    for function in analysis.function_list:
        if function.length > function_length_threshold:
            score_delta -= weight_for(config, "function_length")
            deductions.append(
                Violation(
                    layer="3",
        ...
        if function.cyclomatic_complexity > complexity_threshold:
            score_delta -= weight_for(config, "cyclomatic_complexity")
            deductions.append(
                Violation(
                    layer="3",
        ...
        if function.parameter_count > parameter_threshold:
            score_delta -= weight_for(config, "parameter_count")
            deductions.append(
                Violation(
                    layer="3",
    ...
    if module["path"].startswith("scripts/verify/") or module["path"].startswith(".verifier/"):
        for wrapper_line in pass_through_wrappers(content, module["language"]):
            score_delta -= weight_for(config, "craft")
            deductions.append(
                Violation(
                    layer="3",
...
def _audit_module(
    ...
    if module["line_count"] > file_length_threshold:
        score_delta -= weight_for(config, "file_length")
        deductions.append(
            Violation(
                layer="3",
                rule="file_length",
    ...
    if module["language"] in {"rust", "swift", "python"}:
        delta, code_deductions = _audit_code_module(module, path, content, config)
        score_delta += delta
        deductions.extend(code_deductions)
...
def _collect_module_deductions(
    facts: dict[str, Any],
    repo_root: pathlib.Path,
    config: dict[str, Any],
    ...
    for module in facts["modules"]:
        ...
        if excluded(config, module["path"]):
            continue
        delta, module_deductions = _audit_module(module, repo_root, config)
        score_delta += delta
        deductions.extend(module_deductions)
    return score_delta, deductions
...
def main() -> None:
    ...
    module_delta, module_deductions = _collect_module_deductions(facts, repo_root, config, selected_paths)
    score += module_delta
    deductions.extend(module_deductions)
